...
export default function BookingWidget({ apiKey = 'rv_demo_client_12345' }: BookingWidgetProps) {
  ...
  const { data: servicesList } = trpc.widget.listServices.useQuery(
    { clientId: clientConfig?.id || 1 },
    { enabled: !!clientConfig?.id }
  );
  const { data: vehiclesList } = trpc.widget.listVehicles.useQuery(
    { clientId: clientConfig?.id || 1, passengers: booking.passengers || undefined },
    { enabled: !!clientConfig?.id && currentStep >= 2 }
  );
  ...
  const taxRate = 0.08;
  const subtotal = selectedVehicle ? parseFloat(String(selectedVehicle.basePrice)) : 0;
          ...
          >
              ...
              <div>
                ...
                <div className="space-y-3 mb-4">
                  {vehiclesList?.map(vehicle => (
                    <button
                      key={vehicle.id}
                      onClick={() => updateBooking({ vehicleId: String(vehicle.id) })}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1 mb-2">
                          ...
                          <span className="font-body text-xs text-warm-gray">{vehicle.capacityMin}-{vehicle.capacityMax} pasajeros</span>
                        </div>
                        ...
                        <span className="font-body text-lg font-bold text-terracotta">${parseFloat(String(vehicle.basePrice))} USD</span>
                      </div>
                    </button>
                  ))}
                </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  >
                    {createBooking.isPending ? 'Procesando...' : 'Continuar al Pago'}
                    <ArrowRight size={16} />
                  </button>
                </div>
              </div>
            ...
            {currentStep === 4 && (
              <div>
                ...
                >
                  {createBooking.isPending ? 'Procesando...' : 'Completar Reserva'}
                </button>
              </div>
            )}
          </motion.div>
